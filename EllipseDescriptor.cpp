...
void EllipseDescriptor::update() {
    guidelines[0].position = { target.getSemiMajorRadius(), 0.f };
    guidelines[1].position = { 0.f, 0.f };
    guidelines[2].position = { 0.f, target.getSemiMinorRadius() };
}
...
void EllipseDescriptor::draw(sf::RenderTarget &target, const sf::RenderStates &states) const {
    ...
    sf::Text semi_major_text { std::to_string(static_cast<int>(this->target.getSemiMajorRadius())), text_font, 10U };
    semi_major_text.setOrigin({ semi_major_text.getLocalBounds().width / 2, -2.f });
    semi_major_text.setPosition(this->target.getTransform().transformPoint({ this->target.getSemiMajorRadius() / 2, 0.f }));
    semi_major_text.setRotation(this->target.getRotation());

    sf::Text semi_minor_text { std::to_string(static_cast<int>(this->target.getSemiMinorRadius())), text_font, 10U };
    semi_minor_text.setOrigin({ semi_minor_text.getLocalBounds().width / 2, -2.f });
    semi_minor_text.setPosition(this->target.getTransform().transformPoint({ 0.f, this->target.getSemiMinorRadius() / 2 }));
